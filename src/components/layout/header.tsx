...
export function Header({ locale }: { locale: Locale }) {
  ...
  return <><header className="site-header"><div className="header-inner"><Link href={`/${locale}`} className="site-brand"><span className="brand-mark">A</span><span className="brand-name-full">{d.siteName}</span><span className="brand-name-mobile">{locale === "en" ? "AWS Journal" : "Nhật ký AWS"}</span></Link><nav className="desktop-nav" aria-label="Primary navigation">{nav.map((item) => <Link key={item} href={`/${locale}/${item}`} className={pathname === `/${locale}/${item}` ? "active" : undefined}>{d.nav[item]}</Link>)}</nav><div className="header-actions"><button className="icon-button" type="button" onClick={() => setSearchOpen(true)} aria-label={d.search}><MagnifyingGlass size={19} /></button><ThemeToggle label={d.theme} /><Link className="language-link" href={pathForLocale(pathname, nextLocale)} hrefLang={nextLocale}>{locale === "en" ? "VI" : "EN"}</Link><div className="desktop-auth"><AccountControl locale={locale} /></div><button className="icon-button mobile-menu-button" type="button" onClick={() => setMobileOpen((value) => !value)} aria-label={d.menu} aria-expanded={mobileOpen}>{mobileOpen ? <X size={21} /> : <List size={22} />}</button></div></div>{mobileOpen ? <div className="mobile-drawer"><nav aria-label="Mobile navigation">{nav.map((item) => <Link key={item} href={`/${locale}/${item}`} onClick={() => setMobileOpen(false)}>{d.nav[item]}</Link>)}</nav><div className="mobile-drawer-bottom"><Link href={pathForLocale(pathname, nextLocale)}>{locale === "en" ? "Tiếng Việt" : "English"}</Link><AccountControl locale={locale} /></div></div> : null}</header><SearchDialog locale={locale} open={searchOpen} onClose={() => setSearchOpen(false)} /></>;
}
